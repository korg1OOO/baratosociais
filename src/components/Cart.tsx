    ...
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {!isExpanded && isOpen && (
        <button
          ...
          className="bg-gradient-to-r from-purple-600 to-pink-500 text-white rounded-full p-4 shadow-lg hover:scale-105 transition-all duration-300 flex items-center space-x-2"
        >
          <ShoppingBag className="h-6 w-6" />
          <span className="text-sm font-semibold">
            {items.length} {items.length === 1 ? 'item' : 'itens'} | R$ {total.toFixed(2).replace('.', ',')}
          </span>
        </button>
      )}
